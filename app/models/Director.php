...
class Director {
    ...
    public static function getById(int $id): ?Director {
        $pdo = coneccion();
    ...
    public static function create(string $nombre, string $apellidos, string $fechaNacimiento, string $nacionalidad): bool {
        $pdo = coneccion();
        $stmt = $pdo->prepare("INSERT INTO directores (nombre, apellidos, fecha_nacimiento, nacionalidad) VALUES (:nombre, :apellidos, :fecha_nacimiento, :nacionalidad)");
        return $stmt->execute([
            ":nombre" => $nombre,
            ":apellidos" => $apellidos,
    ...
    public static function update(int $id, string $nombre, string $apellidos, string $fechaNacimiento, string $nacionalidad): bool {
        $pdo = coneccion();
        $stmt = $pdo->prepare("UPDATE directores SET nombre = :nombre, apellidos = :apellidos, fecha_nacimiento = :fecha_nacimiento, nacionalidad = :nacionalidad WHERE id = :id");
        return $stmt->execute([
            ":id" => $id,
            ":nombre" => $nombre,
            ":apellidos" => $apellidos,
